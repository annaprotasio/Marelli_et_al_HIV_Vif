...
- R
- various R packaged (see SessionInfo at the end)
...
for ( i in 1:length(all.file)) { r32 = c(r32, (all.file[[i]][32]))}
...
for ( i in 1:length(all.file)) { r34 = c(r34, (all.file[[i]][34]))}
...
for ( i in 1:length(all.file)) { r136 = c(r136, (all.file[[i]][136]))}
...
mm$res = factor(mm$res, c("I","V","K","R","G","other"))
...
r2 = 136
v = c()
for ( i in 1:length(all.file)) { v = c(v, paste(all.file[[i]][r1],all.file[[i]][r2], sep="")) }
...
all.table = data.frame( 
  res = names(table(all))
  , freq = as.numeric(table(all))
  , prec = round(as.numeric(table(all))/sum(table(all)), digits = 3)
  )

kable(all.table, "html") %>%
  kable_styling(full_width = F, position = "float_left")
```
...
# prepare data
p1 = as.data.frame(table(all))
p1$all = factor(p1$all,levels(p1$all)[c(1,2,4,5,3)])
...
# make plot
p1.plot  = ggplot(data = p1, aes(x = "", y = Freq, fill = all)) + 
  geom_bar(position = "fill", colour = "black", stat = "identity") +
...
Trees' were calculated using Clustal Omega (https://www.ebi.ac.uk/Tools/msa/clustalo/). Briefly, each file containing FASTA  sequences for either VIF, ENV, GAG or NEF were processed for multiple sequence alignment and the output generated in 'phylip' format. These were use as input in 'ggtree' for tree display highlighting the position of the isoalted with selected residue combinations as well as clades. 
...
# extract shared sequences from each protein group
shared.all = list(
  env.shared.all = env[names(env) %in% shared]
  , gag.shared.all = gag[names(gag) %in% shared]
  , nef.shared.all = nef[names(nef) %in% shared]
  , vif.shared.all = vif[names(vif) %in% shared]
) 

# print sequences to file
dir.create("data/all_seqs/out/")
for (i in 1:length(shared.all)) {write.fasta(shared.all[[i]], names(shared.all[[i]]) , paste("data/all_seqs/out/",names(shared.all)[i],"_sharedAll.fasta", sep=""), open = "w", nbchar = 60, as.string = FALSE)}
```


```{bash}
# use clustalO to build phylogeny trees and output to Newick format (that can be read into ggtree)
## IMPORTANT: YOU NEED A WORKING VERSION OF CLUSTALO - see http://www.clustal.org/omega/ for a download. Alternatively, use the webserver to generate Newick files. This step takes quite some time. 
# for i in data/all_seqs/out/*.fasta; do /Users/avp25/bin/miniconda3/bin/clustalo --dealign -i $i -o $i.phylip --outfmt phy --guidetree-out=$i.nwk --force; done
```
...
tree.vif.all = read.tree("data/all_seqs/out/vif.shared.all_sharedAll.fasta.nwk")
tree.gag.all = read.tree("data/all_seqs/out/gag.shared.all_sharedAll.fasta.nwk")
tree.nef.all = read.tree("data/all_seqs/out/env.shared.all_sharedAll.fasta.nwk")
tree.env.all = read.tree("data/all_seqs/out/env.shared.all_sharedAll.fasta.nwk")


c.red = shared[shared %in% names(all.file)[which(all == "II")]] # most active
c.orange = shared[shared %in% names(all.file)[which(all %in% c("IR","VI"))]] # intermediate
c.green = shared[shared %in% names(all.file)[which(all == "VR")]] # inactive

```

A total of **`r length(names(shared.all[[1]]))`** entries are shared across alignments of Env Gag, Nef and Vif. 
...
tree.list.all = list(tree.vif.all, tree.gag.all, tree.nef.all, tree.env.all)
names(tree.list.all) = c("Vif","Gag","Nef","Env")

res.tree = groupOTU(ggtree(tree.list.all[[1]], branch.length='none', layout = 'circular'), grp.cat4, 'Species') +
  aes(color=Species) +
...
There are `r length(tree.list.all$Vif$tip.label)` tips in each tree.


We can compare the phylogenetic distribution of residues with that of clades in the tree. 

```{r residue_clades}
# prepare data for tree. indicate which sequences correspond to each clade.
grp = list(clade.a = shared[grep("^A", shared)]
           , clade.b = shared[grep("^B", shared)]
           , clade.c = shared[grep("^C", shared)]
)
# list all other sequences under "other"
grp$other = shared[shared %!in% c(grp$clade.a,grp$clade.b, grp$clade.c)]

names(grp) = c("A", "B", "C", "other")

clade.tree = groupOTU(ggtree(tree.list.all[[1]], branch.length='none', layout = 'circular'), grp, 'Species') +
  aes(color=Species) +
  scale_color_manual(values=c(viridis(9, option = "A")[c(2,5,8)],"grey")) +
      theme(legend.title = element_blank()
             , legend.position="bottom"
  #          # , plot.title = element_text(hjust = 0.5)
            )
...
```{r wrap_up}
dir.create("Rdata")
save.image("RData/All_clusters.RData")
sessionInfo()
```
